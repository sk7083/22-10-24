-- 장르 등록(최초 한번만 등록)
insert into genre (ge_name) values('드라마'),('판타지'),('서부'),('공포'),('멜로/로맨스'),('어드벤처'),('스릴러'),('느와르'),('컬트'),('다큐멘터리'),
('코미디'),('가족'),('미스터리'),('전쟁'),('애니메이션'),('범죄'),('뮤지컬'),('SF'),('액션'),('무협'),('에로'),('서스펜스'),('서사'),
('블랙코미디'),('실험');
SELECT * FROM GENRE;
-- 영화인 등록(등록되지 않은 영화인이 나오는 경우 등록)
insert into movie_person(mp_name, mp_birth, mp_contry)
    ...
    ('시고니 위버','19491008','미국'),
    ('우나 채플린', '19860604', ''),
    ('지오바니 리비시', '19741217','미국'),
    ('스티븐 랭', '19520711','미국'),
    ('케이트 윈슬렛','19751005','영국');
	
    delete from movie_person;
    ALTER TABLE movie_person auto_increment = 1;

    select * from movie_person;
-- 영화를 등록(movie)
insert into movie (mo_title, mo_contents, mo_age, mo_run, mo_opening_date, mo_state)
	values ('아바타-물의 길', '<아바타: 물의 길>은 판도라 행성에서... ', '12세 이상', 192, '2022-12-14', '현재 상영중'),
('영웅', '동지들과 함께 네 번째 손가락을 자르는 단지동맹으로
조국 독립의 결의를 다진 안중근은
조선 침략의 원흉인 ‘이토 히로부미’를
3년 내에 처단하지 못하면 자결하기로 피로 맹세한다.', '12세 이상', 120, 20221221, '예매중');
