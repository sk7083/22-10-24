...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalName, byte[] 	
		...
		//UUID를 생성(36개 문자와 4개의 하이픈(-))
		// 8-4-4-4-12
		UUID uid = UUID.randomUUID();
		//originalName : 원본 파일명
		//savedName : 서버에 저장될 파일명
		String savedName = uid.toString() +"_" + originalName;
		//savedPath : 서버에 저장할 경로 - 년/월/일  ex) \2023\02\27
		//uploadPath : 저장할 서버 폴더 위치  ex) D:\A
		String savedPath = calcPath(uploadPath);
		//빈 파일 생성 : 서버에 업로드 될 빈 파일을 생성
		File target = new File(uploadPath + savedPath, savedName);
		//복사를 해서 서버에 업로드
		FileCopyUtils.copy(fileData, target);
		//서버에 업로드된 파일의 경로와 파일명
		// /2023/02/27/uuid_flower.jpg	(앞에 D:\A 는 고정 폴더라서 안씀)
	...
	private static String calcPath(String uploadPath) {
		...
		// File.separator <- window 기반인 경우 \ 를 추가해줌,
		//운영체제에 따라 값이 변함
		// \\2023
		String yearPath = File.separator+cal.get(Calendar.YEAR);
		// DecimalFormat("00") <- 두자리를 맞춰줌
		// \\2023\\02
	...
	private static String makeIcon(String uploadPath, String path, String fileName)
		...
		// => /2023/02/27/uuid_flower.jpg (/로 바꿔준다보면 됨)
