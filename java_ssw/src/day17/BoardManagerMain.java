...
public class BoardManagerMain {
	...
	public static void main(String[] args) {
		 ...
		 */
			int menu = 0;
			ArrayList<Board> list = new	ArrayList<Board>();	//<Board> : 담을 곳
	...
	public static void insertBoard(ArrayList<Board> list, Board board) {
		//목록에 추가
		//0번지에 추가하는 이유는 나중에 등록된 게시글이 위로 가게 하기 위해
		list.add(0, board);
	...
	public static Board inputBoard() {
		...
		System.out.println("제목 : ");
		String title = scan.nextLine();//공백이 들어가면 nextLine
		System.out.println("내용 : ");
		String content = scan.nextLine();
		System.out.println("작성자 : ");
		String writer = scan.next();	//아이디는 공백이 없기에 next()
		//목록에 게시글 추가
		//게시글 객체 생성
	...
	public static void printDatail(ArrayList<Board> list, int num) {
		//해당 번호와 맞는 게시글 조회
		int index = list.indexOf(new Board(num));	//Integer num와 Board 비교
